...
int main(int argv, char *argc[]) 
{
  ...
  //----  算法の指定
  if ( argv == 5 ) { sw = atoi(argc[1]); snum = atoi(argc[2]); cnum = atof(argc[3]); }
  else { puts("何か足りないか多い"); }
  if ( sw < 4 || sw > 8 ) {
    puts("実行時引数の不正"); 
    puts("4: シェルソート 5: コームソート"); 
    return 0;
  }
  ...
  while ( m-- > 0 ) {
    ...
    switch ( sw ) {
    case 4:
      shell_sort(arr, n, &ct_cmp, &ct_swp, snum);
      break;
    case 5:
      comb_sort(arr, n, &ct_cmp, &ct_swp, cnum);
      break;
    }
    end = clock();
...
void shell_sort(Data arr[], int n, int *ct_c, int *ct_s, int num) 
{
  int cnt = num;    // 間隔列の収縮係数
  int gap = 1;    // 間隔の最小値
  int p;
...
void comb_sort(int arr[], int n, int *ct_c, int *ct_s, double num) 
{
  double cnt = num;    // 間隔列の収縮係数
  int gap = n;                // 間隔の最大値
  int k;
  ...
  while ( gap > 1 || flag == TRUE ) {
    ...
    //if ( gap == 9 || gap == 10 ) { gap = 11; }    // 経験的な調整(よくわからん)
